...
export class ModalDashboardComponent  {
  ...
  constructor( private common: CommonService, private renderer: Renderer2,public layout:SettingsService) {
    this.common.base.subscribe((res: string) => {
        this.base = res;
        if (this.base === 'layout-mini') {
          layout.changeLayoutMode('mini');
        }else if (this.base === 'layout-hoverview'){
          layout.changeLayoutMode('hoverview');
        }else if (this.base === 'layout-fullwidth'){
...
ngOnInit():void{
  ...
  if(this.base === 'layout-mini'){
    this.layout.SideBarMouseHover=true;
  }
}
ngOnDestroy():void{
  this.renderer.removeClass(document.body,'date-picker')
  this.layout.SideBarMouseHover=false
}
}
